
[![Version][version-shield]](version-url)
[![Contributors][contributors-shield]][contributors-url]
[![Forks][forks-shield]][forks-url]
[![Stargazers][stars-shield]][stars-url]
[![Issues][issues-shield]][issues-url]
[![MIT License][license-shield]][license-url]
<center><img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&height=200&section=header&text=Discord-Bot&fontSize=80&fontAlignY=35&animation=twinkling&fontColor=gradient" /></center>

<br />
<!-- PROJECT LOGO -->
<br />

  <h3 align="center">Discord-Bot</h3>

  <p align="center">
   Versão atualizada de <a href="https://github.com/DotwoodMedia/Dbot">Dbot</a>, um bot multifuncional avançado contendo mais de 400 comandos.<br> Pode fazer Moderação, Ingressos, Rádio, Jogos, Brindes, Personalização, Economia, Nivelamento, Convites, Mensagens, Utilidades, Sugestões, Estatísticas do Servidor etc.<br> Infelizmente os proprietários pararam no pico e decidiram colocar online a fonte do qual fiz uma versão atualizada.
<br>
    <br />
    <br />
    <a href="https://github.com/Krebinkkj/discord-bot/issues">Denuncie um bug</a>
    ·
    <a href="https://github.com/Krebinkkj/discord-bot/issues">Peça um recurso</a>
  </p>
</p>

<!-- NOTICE -->

### <img src="https://cdn.discordapp.com/emojis/1055803759831294013.png" width="20px" height="20px"> 》Noticia 
> Você não pode reivindicar isso como seu! A fonte original foi criada por[Dotwood Media](https://github.com/DotwoodMedia) e [Graphix Development](https://github.com/GraphixDevelopment). A fonte foi modificada e atualizada por mim (Krebinkkj)

> Bot baseado em [Discord.js](https://github.com/Discordjs/discordjs)
Se você gostou deste repositório, fique à vontade para deixar uma ⭐ para me motivar!

<!-- ABOUT THE PROJECT -->

## <img src="https://cdn.discordapp.com/emojis/859424401186095114.png" width="20px" height="20px">》Descrição
[![Readme Card](https://github-readme-stats.vercel.app/api/pin/?username=corwindev&repo=Discord-bot&theme=tokyonight)](https://github.com/Krebinkkj/Discord-bot)
## <img src="https://cdn.discordapp.com/emojis/852881450667081728.gif" width="20px" height="20px">》Recursos
- [x] Slash Commands 
- [x] Upto date with Discord.js v14
- [x] Automod
- [x] Custom Commands
- [x] Comandos de musica
- [x] Tickets
- [x] Comandos de utilidades
- [x] Sugestões 
- [x] Reaction Roles
- [x] Familia (Categoria inutil que fiz)
- [x] Sorteios 
- [x] Facil de usar
- [x] Customizavel
- [x] E muito mais
- [x] Não quer hospedar você mesmo? [Use a versão publica](https://discord.com/api/oauth2/authorize?client_id=860390761307439114&permissions=8&scope=bot%20applications.commands)
<p align="center">
  <a href="https://github.com/Krebinkkj/discord-bot">
    <img src="https://cdn.discordapp.com/attachments/778665159316209748/1055832339328024666/207117434-d98356b1-bf19-418e-9e12-0ef83e0d9a21.png">
  </a>
</p>

## <img src="https://cdn.discordapp.com/emojis/1009754836314628146.gif" width="25px" height="25px">》Requisitos
- NodeJs v17+
- Java v13 para lavalink server.
- Discord Token. Consiga em [Discord Developers Portal](https://discord.com/developers/applications)
- Mongo Database URL. Consiga em [MongoDB](https://cloud.mongodb.com/v2/635277bf9f5c7b5620db28a4#clusters)
- Giphy API Token. Consiga em [Giphy Developers Portal](https://developers.giphy.com/)
- OpenAI API Key `para chatbot`. Get it from [OpenAi Developers Portal](https://beta.openai.com/account/api-keys)
- ClientID `Para carregar comandos em barra (SlashCommands).` [Discord Developers Portal](https://discord.com/developers/applications)
- Spotify client ID `para  suporte com spotify` [Click here to get](https://developer.spotify.com/dashboard/login)
- Spotify client Secret `para  suporte com spotify` [Click here to get](https://developer.spotify.com/dashboard/login)

## <img src="https://cdn.discordapp.com/emojis/814216203466965052.png" width="25px" height="25px">》Guia de instalação

### <img src="https://cdn.discordapp.com/emojis/1028680849195020308.png" width="15px" height="15px"> Installing via [NPM](https://www.npmjs.com/)
Clone o repositório executando
```bash
git clone https://github.com/Krebinkkj/Discord-Bot.git
```
### Após a clonagem preencha todos os requisitos em `.env` **(renomeie `.env.example` para `.env`)**, então execute

```bash
npm install
```
Para iniciar o bot

```js
node src/index.js
```

# <img src="https://cdn.discordapp.com/emojis/1015745034076819516.png" width="25px" height="25px">》Faq
> Como obter acesso aos comandos dos desenvolvedores? Você terá que configurá-los via MongoDB ou executar o comando abaixo.

```bash
npm run add-dev YOUR_Discord_ID
```
# Donate

 Ao doar, você me ajudará a manter este projeto

<img src="https://cdn.discordapp.com/emojis/809085860632985630.png" width="15px" height="15px"> 》[Sponsor](https://github.com/sponsors/Krebinkkj)

[version-shield]: https://img.shields.io/github/package-json/v/Krebinkkj/Discord-Bot?style=for-the-badge
[version-url]: https://github.com/brblacky/WaveMusic
[contributors-shield]: https://img.shields.io/github/contributors/Krebinkkj/Discord-Bot.svg?style=for-the-badge
[contributors-url]: https://github.com/Krebinkkj/Discord-Bot/graphs/contributors
[forks-shield]: https://img.shields.io/github/forks/Krebinkkj/Discord-Bot.svg?style=for-the-badge
[forks-url]: https://github.com/Krebinkkj/Discord-Bot/network/members
[stars-shield]: https://img.shields.io/github/stars/Krebinkkj/Discord-Bot.svg?style=for-the-badge
[stars-url]: https://github.com/Krebinkkj/Discord-Bot/stargazers
[issues-shield]: https://img.shields.io/github/issues/Krebinkkj/Discord-Bot.svg?style=for-the-badge
[issues-url]: https://github.com/Krebinkkj/Discord-Bot/issues
[license-shield]: https://img.shields.io/github/license/Krebinkkj/Discord-Bot.svg?style=for-the-badge
[license-url]: https://github.com/Krebinkkj/Discord-Bot/blob/master/LICENSE
