...
const logModeration = async (issuer, target, reason, type, data = {}) => {
  ...
  switch (type.toUpperCase()) {
    case "PURGE":
      embed.setAuthor({ name: `Moderação - ${type}` });
      fields.push(
  ...
  if (type.toUpperCase() !== "PURGE") {
    embed.setAuthor({ name: `Moderação - ${type}` }).setThumbnail(target.displayAvatarURL());

    if (target instanceof GuildMember) {
      fields.push({ name: "Membro", value: `${target.displayName} [${target.id}]`, inline: false });
    } else {
      fields.push({ name: "User", value: `${target.tag} [${target.id}]`, inline: false });
    }

    fields.push({ name: "Motivo", value: reason || "Nenhum motivo providenciado", inline: false });
    ...
    if (type.toUpperCase() === "MOVE") {
      fields.push({ name: "Movido para", value: data.channel.name, inline: true });
    }
  }
