...
async function performAutomod(message, settings) {
  ...
  if (mentions.members.size > automod.max_mentions) {
    fields.push({ name: "Menções", value: `${mentions.members.size}/${automod.max_mentions}`, inline: true });
  ...
  if (mentions.roles.size > automod.max_role_mentions) {
    fields.push({ name: "Menções de cargos", value: `${mentions.roles.size}/${automod.max_role_mentions}`, inline: true });
  ...
  if (automod.anti_massmention > 0) {
    ...
    if (mentions.everyone) {
      fields.push({ name: "Menção **Everyone**", value: "✓", inline: true });
      strikesTotal += 1;
    }
  ...
  if (automod.max_lines > 0) {
    ...
    if (count > automod.max_lines) {
      fields.push({ name: "Novas linhas", value: `${count}/${automod.max_lines}`, inline: true });
      shouldDelete = true;
  ...
  if (automod.anti_attachments) {
    if (message.attachments.size > 0) {
      fields.push({ name: "Anexos encontrados", value: "✓", inline: true });
      shouldDelete = true;
      strikesTotal += 1;
    }
  }
  ...
  if (automod.anti_links) {
    if (containsLink(content)) {
      fields.push({ name: "Links encontrados", value: "✓", inline: true });
      shouldDelete = true;
      strikesTotal += 1;
    }
  }
    ...
    if (containsLink(content)) {
      ...
      if (antispamCache.has(key)) {
        ...
        if (
          ...
          fields.push({ name: "Detecção AntiSpam", value: "✓", inline: true });
          shouldDelete = true;
          strikesTotal += 1;
        }
      } else {
  ...
  if (shouldDelete && message.deletable) {
    message
      .delete()
      .then(() => channel.safeSend("> Automod! Mensagem apagada", 5))
      .catch(() => {});
  }
  ...
  if (strikesTotal > 0) {
    ...
    if (logChannel) {
      const logEmbed = new EmbedBuilder()
        .setAuthor({ name: "Auto Moderatção" })
    ...
    const strikeEmbed = new EmbedBuilder()
      ...
      .setAuthor({ name: "Auto Moderação" })
      .addFields(fields)
      .setDescription(
        `Você recebeu ${strikesTotal} strikes!\n\n` +
          `**Servidor:** ${guild.name}\n` +
          `**Total de Strikes:** ${memberDb.strikes} fora de ${automod.strikes}`
      );
    ...
    if (memberDb.strikes >= automod.strikes) {
      ...
      await addModAction(guild.members.me, member, "Automod: Max de strikes recebido", automod.action).catch(() => {});
    }
