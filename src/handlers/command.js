...
module.exports = {
  ...
  handlePrefixCommand: async function (message, cmd, settings) {
    ...
    if (cmd.category === "OWNER" && !OWNER_IDS.includes(message.author.id)) {
      return message.safeReply("Esse comando é acessivel apenas para o desenvolvedor");
    }
    ...
    if (cmd.botPermissions && cmd.botPermissions.length > 0) {
      if (!message.channel.permissionsFor(message.guild.members.me).has(cmd.botPermissions)) {
        return message.safeReply(`Eu preciso ${parsePermissions(cmd.botPermissions)} para esse comando`);
      }
    }
    ...
    if (cmd.cooldown > 0) {
      ...
      if (remaining > 0) {
        return message.safeReply(`Você está em cooldown. Use o comando denovo em \`${timeformat(remaining)}\``);
      }
    }
    ...
    try {
      ...
      message.safeReply("Ocorreu um erro executando essa mensagem");
    } finally {
  ...
  handleSlashCommand: async function (interaction) {
    ...
    if (!cmd) return interaction.reply({ content: "Ocorreu um erro", ephemeral: true }).catch(() => {});
    ...
    if (cmd.category === "OWNER" && !OWNER_IDS.includes(interaction.user.id)) {
      return interaction.reply({
        content: `Esse comando é acessivel apenas para o desenvolvedor!`,
        ephemeral: true,
      });
    }
    ...
    if (cmd.botPermissions && cmd.botPermissions.length > 0) {
      if (!interaction.guild.members.me.permissions.has(cmd.botPermissions)) {
        return interaction.reply({
          content: `Eu preciso ${parsePermissions(cmd.botPermissions)} para esse comando`,
          ephemeral: true,
        });
      }
    }
    ...
    if (cmd.cooldown > 0) {
      ...
      if (remaining > 0) {
        return interaction.reply({
          content: `Você está em cooldown. Use o comando denovo em \`${timeformat(remaining)}\``,
          ephemeral: true,
        });
      }
    }
    ...
    try {
      ...
      await interaction.followUp("Oops! Ocorreu um erro durante a execução do comando!");
