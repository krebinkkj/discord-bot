const {
  ...
  async function closeTicket(channel, closedBy, reason) {
    ...
    try {
      ...
      let content = "";
      reversed.forEach((m) => {
        content += `[${new Date(m.createdAt).toLocaleString("pt-BR")}] - ${m.author.username}\n`;
        if (m.cleanContent !== "") content += `${m.cleanContent}\n`;
        if (m.attachments.size > 0) content += `${m.attachments.map((att) => att.proxyURL).join(", ")}\n`;
        content += "\n";
      });
  
      const logsUrl = await postToBin(content, `Ticket Logs para ${channel.name}`);
      ...
      const embed = new EmbedBuilder().setAuthor({ name: "Ticket Fechado" }).setColor(TICKET.CLOSE_EMBED);
      const fields = [];
  
      if (reason) fields.push({ name: "Motivo", value: reason, inline: false });
      fields.push(
        {
          name: "Aberto por",
  ...
  async function closeAllTickets(guild, author) {
    ...
    for (const ch of channels) {
      const status = await closeTicket(ch[1], author, "Forçar o fechamento de todos os tickets abertos");
  ...
  async function handleTicketOpen(interaction) {
    ...
    if (!guild.members.me.permissions.has(OPEN_PERMS))
      return interaction.followUp(
        "Não é possível criar um canal de tickets, faltando a permissão `Gerenciar canal`. Entre em contato com o gerente do servidor para obter ajuda!"
      );
  
    const alreadyExists = getExistingTicketChannel(guild, user.id);
    if (alreadyExists) return interaction.followUp(`Você já tem um ticket aberto`);
  
    const settings = await getSettings(guild);
  
    // limit check
    const existing = getTicketChannels(guild).size;
    if (existing > settings.ticket.limit) return interaction.followUp("Há muitos tickets abertos. Tente novamente mais tarde");
    ...
    if (categories.length > 0) {
      ...
      await interaction.followUp({ content: "Escolha uma categoria do ticket", components: [menuRow] });
      ...
      if (!res) return interaction.editReply({ content: "Tempo limite esgotado. Tente novamente", components: [] });
      await interaction.editReply({ content: "Processando", components: [] });
    ...
    try {
      ...
      const embed = new EmbedBuilder()
        ...
        .setFooter({ text: "Você pode fechar seu ticket a qualquer momento clicando no botão abaixo" });
      ...
      await interaction.editReply(`Ticket criado! 🔥`);
    } catch (ex) {
      error("handleTicketOpen", ex);
      return interaction.editReply("Falha ao criar canal de ticket, ocorreu um erro!");
    }
  }
  ...
  async function handleTicketClose(interaction) {
    ...
    if (status === "MISSING_PERMISSIONS") {
      return interaction.followUp("Não é possível fechar o ticket, pois faltam permissões. Entre em contato com o gerente do servidor para obter ajuda!");
